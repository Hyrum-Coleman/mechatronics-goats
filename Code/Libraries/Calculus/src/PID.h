...
class PID
{
  private:
    ...
    double m_h;             //!< Gain factor for output
  ...
  public:
     ...
     *  \param [in] h Gain factor for output term
     */
    PID(double p, double i, double d, double h = 1.0);

      /**
     *  \brief Runs the PID controller and calls the moveMotor function
     *  
     *  \param [in] t New sample time
     *  \param [in] u New data sample in time series
     *  \param [in] moveMotors User defined function taking action on the scaled error
