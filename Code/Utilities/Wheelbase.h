...
public:
  ...
  void computeVelocities(float wheelSpeeds[4], float &vx, float &vy, float &omega) {
    ...
    // Calculate vx, vy, and omega based on the inverse kinematics matrix
    /* (r/4)
    //   1          1          1          1
    //  -1          1          1         -1
    ...
    */
    ...
    vy = (r / 4) * (-wheelSpeeds[0] + wheelSpeeds[1] + wheelSpeeds[2] - wheelSpeedes[3]);
    omega = (r / (4 * factor)) * (-wheelSpeeds[0] + wheelSpeedes[1] - wheelSpeeds[2] + wheelSpeedes[3]);
  }
};
